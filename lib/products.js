...
function normalizeRow(row) {
  ...
  const seoTitle = row['seo_title'] || row['SEO Title'] || '';
  const metaDescription = row['meta_description'] || row['Meta Description'] || '';
  const imageAlt = row['image_alt'] || row['Image Alt'] || '';
  ...
  return {
    ...
    image_alt: imageAlt || (row['Product'] || row.product || ''),
  };
}

export async function getProducts() {
  const csvText = await fs.readFile(dataPath, 'utf8');
  const records = parse(csvText, { columns: true, skip_empty_lines: true, relax_column_count: true, bom: true });
